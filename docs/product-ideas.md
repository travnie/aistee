...
8. **Native/API modes and jobs:** keep the normal chat simple, but expose a small provider-aware set of useful processing choices. Per-chat OpenAI Auto/Standard/Flex/Fast is shipped. **OpenAI Background Responses are now shipped as a separate Jobs workflow** with durable local status, WorkManager polling, cancellation and automatic Markdown result capture into Project Library while keeping `store=false`. Next extend the same job model to Gemini Batch and Anthropic Message Batches where their provider APIs support status/cancel/result retrieval. Avoid a generic wall of sampling sliders; add controls only when they have a clear cost, latency or workflow benefit.
...
### Follow-up: Kimi, Mobile Harness, Muse, Qwen and Venice APKs (26 September 2026)

Statically inspected `com.moonshot.kimichat` 3.1.3, `com.jarves.mh` 1.0.4, `com.facebook.aura` 9.0.0.11.178, `com.tongyi.intl` 2.0.4 and `com.ai.venice` 1.17.14. Evidence comes from manifests, selected resources and packaged assets. Bundled strings or a registered Android component do not prove an enabled feature, subscription entitlement, working endpoint or local execution. These APKs are not copied into the repo.

| App and evidence | Candidate / boundary for Aistee |
| --- | --- |
| **Kimi 3.1.3:** `ExternalLinkActivity` accepts `SEND`, `SEND_MULTIPLE` and `PROCESS_TEXT` for documents (including PDF/Office/ePub/Markdown), with a separate image alias. Launcher shortcuts open camera or voice call. Resources name agent status, messages, cancellation and used-credit warnings; they also mention Kimi Claw/OpenClaw group chats. | For Aistee's native jobs, show running/stopped/failed status and whether cancellation affects already-used provider credit. Existing Projects, skills and Markdown export notes cover the older Kimi patterns. OpenClaw-related labels are product integration hints, not a portable API or a reason to require a remote Claw. |
| **Mobile Harness 1.0.4:** the APK packages an arm64 Linux runtime archive, `libproot.so` and app text describing a private project workspace with Chat, Terminal, Changes and Preview; it names Claude Code/Antigravity integration. The manifest has no external share target or widget and declares package-install capability. | A self-contained coding workspace is a *separate, high-scope* product direction. If explored later, require a visible project root, explicit file/command scope and review of changes; do not run imported Aistee skills or ordinary chat tools in a bundled shell. APK text cannot prove the runtime sandbox or provider auth is secure. |
| **Muse from Meta:** `com.facebook.aura` registers assistant/voice services, `PROCESS_TEXT` and text/image/multiple-file share entry points. Assets contain distinct DOCX, PPTX and spreadsheet viewers plus a VNC canvas. The manifest also requests broad device, location and Health Connect permissions. | Aistee can reuse the *idea* of format-specific previews in its local Project Library. Keep each device capability behind its own consent and data boundary; do not broaden Aistee's default permissions to match an assistant app. Bundled viewers do not prove offline editing or availability for every account. |
| **Qwen 2.0.4:** four static launcher shortcuts are named for meeting, photo question, speaking practice and text-to-image; no equivalent app-widget receiver appears in this manifest pass. Bundled scan/filter configs and resources mention document capture, table-to-spreadsheet export, PDF/Word conversion, and some cloud-save warnings. | Use task-named shortcuts only for actions Aistee actually supports. Document scanning and conversion belong to explicit Docbench-style tools with input preview, export destination and honest local/cloud labeling. Many strings appear to come from a bundled Quark scan module, so they cannot establish Qwen chat/API capability. |
| **Venice AI 1.17.14:** the Android manifest exposes app/deep-link entry points, with no public `SEND`/`PROCESS_TEXT` target or widget receiver in this APK. `allowBackup=false`; its network XML disallows cleartext by default and includes localhost/development-host exceptions. | Keep Venice Web/account verification in the provider matrix. The APK does not establish an Android sharesheet flow, a native API contract or a privacy guarantee for provider-side storage; Aistee's local assets should not be uploaded merely because a provider app exists. |

**Actionable ordering:** apply the Kimi-style cancellation/credit explanation to durable native jobs first; consider format-specific previews when Project Library intake supports them. Treat the device assistant and local coding-runtime directions as separate, explicitly scoped proposals rather than new requirements for normal chat.

...
- **Shipped:** Studio tools → Privacy holds two opt-in, device-local controls. Screen privacy applies `FLAG_SECURE` to every Aistee activity through application-level lifecycle callbacks. App lock covers every activity with a biometric/device-credential check on a fresh process or after a minute away; short trips such as file pickers keep the unlock, and it stays inactive without a secure lock screen. A quick notification/widget privacy mode remains open.

# Third-party notices

Aistee includes the following third-party software.

## JTokkit 1.1.0

- Project: JTokkit by Knuddels GmbH & Co. KG / contributors
- Purpose in Aistee: local `o200k_base` token counting on Android
- License: MIT
- Copyright: © 2023 Knuddels, Philip Müller

The complete JTokkit MIT license text is bundled in Android release artifacts at `assets/licenses/JTokkit-LICENSE.txt` and is also available in the upstream JTokkit repository.
